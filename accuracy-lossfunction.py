...
# Step 5: Calculate Accuracy and Loss on Test Data

# Calculate predictions on test set
test_predictions = model.predict(test_set, steps=len(test_set))
test_predictions_binary = (test_predictions > 0.5).astype(int)

# Extract true labels from the test set
true_labels = np.concatenate([test_set[i][1] for i in range(len(test_set))])

# Calculate accuracy
accuracy = np.mean(np.equal(true_labels, test_predictions_binary.squeeze()))
print(f'Accuracy on Test Data: {accuracy:.4f}')

# Calculate loss
test_loss = model.evaluate(test_set, steps=len(test_set), verbose=0)[0]
